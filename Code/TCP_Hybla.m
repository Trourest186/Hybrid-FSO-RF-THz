...
%    Pe(k,1)= Pt_FSO_RF(k-1,SNR_thres(k,1));
   Wmax =sqrt(((8*ph^2)*(1-exp(-(Pe(k,1)*3*W^2)/(8*ph^2))))/(3*Pe(k,1)));
   lamda(k,1) = (2*ph^2)/(RTT*Wmax);
%     if k ==16 
%         lamda(k,1) = 0.0400;
%     end
%         if k ==16
...
%Stationary Distribution
   for i=1:N
        for j=1:N
            a=(i-0.5)*(W/N);
%             L=(((beta)*a)/(alpha))^(1/3);    
            t=max(0,((((j-1)*W/N)-beta*a)/ph^2)*RTT);
            T(i,j)=t;
            s=(((ph*t)^2)/(2*RTT)) + beta*a*t;
            S(i,j)=s;
              if j < beta*(i-0.5)
                 p=0;
              else
                 p=exp(-lamda(k,1)*(max(0,RTT*((((j-1)*W/N)-beta*a)/ph^2))))- exp(-lamda(k,1)*RTT*((((j)*W/N)-beta*a)/ph^2));           
              end  
           A(i,j)=p;
         end
   end
